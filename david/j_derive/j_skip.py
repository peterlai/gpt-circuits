...
def get_jacobian_mlp_sae(
    ...
    # jacobian_skip_path = einops.einsum(sae_mlpin.W_dec, sae_mlpout.W_enc, 
    #                                    "feat_size_out n_embd, n_embd feat_size_in -> feat_size_out feat_size_in")
    
    # print(f"jacobian_skip_path: {jacobian_skip_path.shape}")
    # print(f"topk_indices_mlpout: {topk_indices_mlpout.shape}")
    # print(f"topk_indices_mlpin: {topk_indices_mlpin.shape}")
...
def get_jacobian_skip(
    ...
    jacobian_skip_path = eindex(jacobian_skip_path, topk_indices_mlpout, topk_indices_mlpin,
                                "[batch seq k2] [batch seq k1] -> batch seq k2 k1")
    return jacobian_skip_path

def get_jacobian_resid(
    sae_mlpin,
    sae_mlpout,
    mlp: MLP,
    topk_indices_mlpin: Int[Tensor, "batch seq k"],
    ...
    mlp_path = get_jacobian_mlp_sae(sae_mlpin, sae_mlpout, mlp, topk_indices_mlpin, topk_indices_mlpout, mlp_act_grads, norm_act_grads)
    skip_path = get_jacobian_skip(sae_mlpin, sae_mlpout, topk_indices_mlpin, topk_indices_mlpout)
    jacobian = mlp_path + skip_path
    return jacobian

def skip_path(in_feat_mags : Float[Tensor, "batch seq feat"],
            sae_mlpin : DummySAE,
            mlp : MLP,
            sae_mlpout : DummySAE
) -> tuple[Float[Tensor, "batch seq feat"], Int[Tensor, "batch seq feat"]]:
    """
    does the skip path
    """
    top_k_values, in_feat_idx = torch.topk(in_feat_mags, sae_mlpin.k, dim=-1)
    ...
    out_feat_mags, out_feat_idx = sae_mlpout.encode(resid_mid, return_indicies=True)
...
def sandwich_for_skip(input_features):
    # We are differentiating the *final sparse output magnitudes*
    # w.r.t the *initial dense input feature magnitudes*.
    sparse_output_mags, _, _ = skip_path(input_features, sae_mlpin, mlp, sae_mlpout)
    return sparse_output_mags
...
sparse_feat_mags_out_skip, in_feat_idx_skip, out_feat_idx_skip = skip_path(sparse_feat_mags_in, sae_mlpin, mlp, sae_mlpout)

jacobian_skip = get_jacobian_skip(
    sae_mlpin,
    sae_mlpout,
    in_feat_idx_skip,
    out_feat_idx_skip,
...
jacobian_torch_mlp_path = autograd_F.jacobian(
...
jacobian_torch_skip_path = autograd_F.jacobian(
    sandwich_for_skip,
    sparse_feat_mags_in,
    create_graph=True,
    vectorize=False,
...
print(f'{jacobian_torch_mlp_path.shape=}')

from eindex import eindex
jacobian_torch_mlp_path = einops.einsum(jacobian_torch_mlp_path, "b s k2 b s k1 -> b s k2 k1") #take diagonal along batch and seq
jacobian_torch_mlp_path_block = eindex(jacobian_torch_mlp_path, out_feat_idx, in_feat_idx, "b s [b s k2] [b s k1] -> b s k2 k1")
...
jacobian_torch_skip_path = einops.einsum(jacobian_torch_skip_path, "b s k2 b s k1 -> b s k2 k1") #take diagonal along batch and seq
jacobian_torch_skip_path_block = eindex(jacobian_torch_skip_path, out_feat_idx_skip, in_feat_idx_skip, "b s [b s k2] [b s k1] -> b s k2 k1")
print(f'{jacobian_torch_skip_path_block.shape=}')
print(f'{jacobian_skip.shape=}')
torch.testing.assert_close(jacobian_skip, jacobian_torch_skip_path_block, rtol=1e-4, atol=1e-4)
print("Results are close!")
